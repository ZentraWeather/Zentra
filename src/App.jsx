...
const FAV_STORAGE_KEY = "zentraweather:favorites:v1";
...
function safeLoadFavorites() {
  ...
  try {
    ...
    return parsed
      .map((f) => ({
        id: String(f.id || ""),
        name: f.name && typeof f.name === "object" ? f.name : null,
        lat: Number(f.lat),
        lon: Number(f.lon)
      }))
      .filter((f) => f.id && Number.isFinite(f.lat) && Number.isFinite(f.lon) && f.name);
  } catch {
    return [];
  }
}

function safeSaveFavorites(favs) {
  if (typeof window === "undefined") return;
  try {
...
function estimateConfidence({ rainProb, windSpeed }) {
  // Note: rainProb (precipitation_probability) est issu d’ensemble météo (30 simulations) :contentReference[oaicite:1]{index=1}
  const midUncertainty = 1 - Math.abs((rainProb ?? 50) - 50) / 50;
  const windPenalty = clamp((windSpeed ?? 0) / 70, 0, 1);
  const score = clamp(0.87 - 0.38 * midUncertainty - 0.22 * windPenalty, 0.25, 0.96);
  if (score >= 0.72) return "high";
  if (score >= 0.5) return "medium";
  return "low";
}

function confidencePill(conf, darkMode) {
  const base = "inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-extrabold";
  const dot = conf === "high" ? "bg-emerald-500" : conf === "medium" ? "bg-amber-400" : "bg-rose-500";
  const shell = darkMode ? "bg-white/6 border border-white/10" : "bg-white/70 border border-white/60";
  return { base, dot, shell };
}

function closestTimeIndex(times, targetIso) {
...
function generateRealisticWeather() {
  ...
  for (let i = 0; i < 48; i++) {
    const d = new Date(topHour);
    d.setHours(d.getHours() + i);
    hourly.time.push(d.toISOString());
    ...
    const ws = 10 + Math.random() * 20;
    hourly.wind_speed_10m.push(ws);
    hourly.wind_gusts_10m.push(ws + 5 + Math.random() * 15);
...
export default function App() {
  ...
  // Favoris
  const [favorites, setFavorites] = useState(() => safeLoadFavorites());
  ...
  // Persist favoris
  useEffect(() => {
    safeSaveFavorites(favorites);
  }, [favorites]);

  // Reset dismiss alert quand nouvelle data / nouvelle position
  useEffect(() => {
    setAlertsDismissed(false);
  }, [location.lat, location.lon, weatherData?.current?.time]);

  useEffect(() => {
    loadWeather();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.lat, location.lon, useRealAPI]);
    ...
    try {
      ...
      if (useRealAPI) {
        ...
        url.searchParams.set(
          "current",
          ["temperature_2m", "relative_humidity_2m", "apparent_temperature", "precipitation", "weather_code", "wind_speed_10m"].join(",")
        );

        // 👇 Ajout wind_gusts_10m pour alertes plus réalistes
        url.searchParams.set(
          "hourly",
          ["temperature_2m", "precipitation_probability", "precipitation", "weather_code", "wind_speed_10m", "wind_gusts_10m"].join(",")
        );

        url.searchParams.set(
          "daily",
          ["weather_code", "temperature_2m_max", "temperature_2m_min", "precipitation_probability_max", "precipitation_sum", "wind_speed_10m_max", "uv_index_max"].join(",")
        );
  ...
  async function useMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          name = { ...name, ...(await reverseGeocodeCity(lat, lon)) };
        } catch {}
  ...
  // Favoris helpers
  const currentFavId = useMemo(() => makeFavId(location.lat, location.lon), [location.lat, location.lon]);
  const isFavorite = useMemo(() => favorites.some((f) => f.id === currentFavId), [favorites, currentFavId]);
  ...
  function toggleFavorite() {
    ...
    const name = location?.name || makeNameObj("Favori");
    const fav = { id, name, lat: Number(location.lat), lon: Number(location.lon) };
    setFavorites((prev) => [fav, ...prev.filter((f) => f.id !== id)].slice(0, 20));
  }
  ...
  // IA Analysis + Confiance
  const aiAnalysis = useMemo(() => {
    if (!weatherData?.current || !weatherData?.daily) return { emoji: "🌤️", text: "", conf: "high", confLabel: "" };
    ...
    if (rainProb > 70) { emoji = "☔"; text = `${rainProb}% ${t.rainPossible}. ${temp}°C, ${weatherDesc.toLowerCase()}.`; }
    else if (rainProb > 40) { emoji = "🌦️"; text = `${rainProb}% ${t.rainPossible}. ${temp}°C, ${weatherDesc.toLowerCase()}.`; }
    else if (temp < 3) { emoji = "❄️"; text = `${temp}°C, ${weatherDesc.toLowerCase()}.`; }
    else if (windSpeed > 35) { emoji = "💨"; text = `${windSpeed} km/h ${t.windPossible}. ${temp}°C, ${weatherDesc.toLowerCase()}.`; }
    else if (temp > 20) { emoji = "☀️"; text = `${temp}°C, ${weatherDesc.toLowerCase()}.`; }
    else { emoji = "🌤️"; text = `${temp}°C, ${weatherDesc.toLowerCase()}.`; }

    return { emoji, text, conf, confLabel };
  ...
  // Indices horaires “à partir de maintenant”
  const hourStartIndex = useMemo(() => {
    const times = weatherData?.hourly?.time || [];
    const target = weatherData?.current?.time || null;
    return closestTimeIndex(times, target);
  }, [weatherData?.hourly?.time, weatherData?.current?.time]);

  // Alertes (pluie/vent) sur les prochaines heures (à partir de now)
  const alerts = useMemo(() => {
    if (!weatherData?.hourly) return null;

    const h = weatherData.hourly;
    const start = hourStartIndex;
    const end = start + 6;

    const ppArr = (h.precipitation_probability || []).slice(start, end).map((x) => Number(x) || 0);
    const prArr = (h.precipitation || []).slice(start, end).map((x) => Number(x) || 0);
    const wsArr = (h.wind_speed_10m || []).slice(start, end).map((x) => Number(x) || 0);
    const wgArr = (h.wind_gusts_10m || []).slice(start, end).map((x) => Number(x) || 0);

    const ppMax = ppArr.length ? Math.max(...ppArr) : 0;
    const prSum = prArr.reduce((a, b) => a + b, 0);
    const windMax = wsArr.length ? Math.max(...wsArr) : 0;
    const gustMax = wgArr.length ? Math.max(...wgArr) : 0;

    // Seuils simples (tweakable)
    const rainSoon = ppMax >= 70 || prSum >= 1.2;
    const windSoon = gustMax >= 55 || windMax >= 45;

    if (!rainSoon && !windSoon) return null;

    const kind = rainSoon && windSoon ? "both" : rainSoon ? "rain" : "wind";

    return {
      kind,
      ppMax: Math.round(ppMax),
      prSum: Math.round(prSum * 10) / 10,
      windMax: Math.round(windMax),
      gustMax: Math.round(gustMax)
    };
  }, [weatherData, hourStartIndex]);
  ...
  const confPill = confidencePill(aiAnalysis.conf, darkMode);
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className={`mb-4 rounded-2xl border shadow-sm p-3 sm:p-4 ${theme.card}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className={`text-sm font-extrabold ${theme.text}`}>{t.alertsTitle}</div>
                  <div className={`text-sm ${theme.muted}`}>
                    {alerts.kind === "both" && (
                      <span>
                        {t.alertRain} ({alerts.ppMax}% / {alerts.prSum}mm) · {t.alertWind} ({alerts.gustMax || alerts.windMax} km/h)
                      </span>
                    )}
                    {alerts.kind === "rain" && (
                      <span>
                        {t.alertRain} ({alerts.ppMax}% / {alerts.prSum}mm)
                      </span>
                    )}
                    {alerts.kind === "wind" && (
                      <span>
                        {t.alertWind} ({alerts.gustMax || alerts.windMax} km/h)
                      </span>
                    )}
                  </div>
                  <div className={`text-xs mt-1 ${theme.muted2}`}>
                    {t.alertsHint}
                  </div>
                </div>
              </div>
              <button
                ...
                className={`rounded-xl border px-3 py-2 text-xs font-extrabold transition ${theme.cardSoft} ${theme.text}`}
              >
                {t.dismiss}
              </button>
            </div>
        ...
        <div className="text-center mb-6 pt-6">
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            <h1 className={`text-3xl sm:text-4xl font-extrabold ${theme.text} flex-1`}>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
            ...
            <span className={`${confPill.base} ${confPill.shell} ${theme.text}`}>
              <span className={`h-2 w-2 rounded-full ${confPill.dot}`} />
              {t.confidenceLabel}: {aiAnalysis.confLabel}
            </span>
          </div>
        </div>
        ...
        <div className={`rounded-2xl border shadow-lg p-4 mb-6 ${theme.card}`}>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between gap-3">
              <div className={`text-sm font-extrabold mb-2 ${theme.text}`}>{t.favoritesTitle}</div>
              <button
                onClick={toggleFavorite}
                className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-extrabold transition ${theme.cardSoft} ${theme.text}`}
            ...
            {favorites.length === 0 ? (
              <div className={`text-sm ${theme.muted2}`}>{t.favoritesEmpty}</div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {favorites.map((f) => (
                  <div key={f.id} className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-extrabold ${theme.cardSoft} ${theme.text}`}>
                    <button onClick={() => goToFavorite(f)} className="hover:underline">
                      {f?.name?.[language] || f?.name?.fr || "Favori"}
                    </button>
                    <button
                      onClick={() => removeFavorite(f.id)}
                      className={`rounded-full p-1 ${darkMode ? "hover:bg-white/10" : "hover:bg-black/5"}`}
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-extrabold">
                  <span className={`h-2 w-2 rounded-full ${aiAnalysis.conf === "high" ? "bg-emerald-300" : aiAnalysis.conf === "medium" ? "bg-amber-300" : "bg-rose-300"}`} />
                  {t.confidenceLabel}: {aiAnalysis.confLabel}
                </span>
              </div>
        ...
        <div className={`rounded-2xl border shadow-xl p-4 sm:p-8 mb-6 ${theme.card}`}>
          ...
          <div className="overflow-x-auto">
            <div className="flex gap-3 pb-4">
              {(hourly.time || []).slice(hourStartIndex, hourStartIndex + 48).map((time, offset) => {
                ...
                return (
                  <div
                    key={`${time}-${index}`}
                    className={`flex-shrink-0 w-24 p-4 rounded-2xl text-center border ${
                      isNow
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center gap-1 text-xs text-sky-500 font-bold mb-1">
