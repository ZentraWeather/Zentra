        ...
        <div className={`rounded-2xl border shadow-xl p-4 sm:p-8 mb-6 ${theme.card}`}>
          ...
          <div className="space-y-4">
            {(daily.time || []).slice(0, 7).map((date, index) => {
              ...
              const minOffset = ((minTemp - tempScale.lo) / tempScale.range) * 100;
              const maxOffset = ((maxTemp - tempScale.lo) / tempScale.range) * 100;
              ...
              return (
                <div key={date} className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 relative h-10 flex items-center">
                    <div className={`absolute inset-0 rounded-full ${darkMode ? "bg-slate-800/60" : "bg-slate-100"}`}></div>

                    <div
                      className="absolute h-8 rounded-full flex items-center justify-between px-2 transition-all
                                 bg-gradient-to-r from-sky-400 via-amber-400 to-rose-400"
